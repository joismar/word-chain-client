...
export function Keyboard() {
    ...
    const rows = [
      ...
      return (
        <div className="flex flex-col items-center space-y-1 w-full p-2">
      {rows.map((row, rowIndex) => (
        <div key={rowIndex} className="flex justify-center space-x-1 w-full">
          {row.map((key) => (
            <button
              key={key}
              className={`${keyBg(key)} hover:bg-neutral-600 text-neutral-100 font-bold rounded aspect-[3/4] w-full max-w-[10%]`}
